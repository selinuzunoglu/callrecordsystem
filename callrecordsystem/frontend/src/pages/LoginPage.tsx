...
function LoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            const response = await axios.post<{ token: string, username: string, role: string, permissions: string[] }>(
            ...
            if (response.data.token) {
                ...
                localStorage.setItem("username", response.data.username); // backend'den dönen gerçek username
                localStorage.setItem("role", response.data.role || "user"); // fallback
                localStorage.setItem("permissions", JSON.stringify(response.data.permissions || [])); // permissions'ı kaydet
